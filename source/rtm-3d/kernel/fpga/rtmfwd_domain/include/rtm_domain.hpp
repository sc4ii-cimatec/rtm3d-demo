...
#ifndef XF_RTM_RTM_Domain_HPP
#define XF_RTM_RTM_Domain_HPP

/**
 * @file rtm.hpp
 * @brief class RTMDomain derived from Stencil3D is defined here, it provides L1 primary
...
/**
 * @brief RTMDomain class defines the basic operations for 2D RTMDomain
 *
 ...
 * @tparam t_PEZ is the number of processing elements
 */
...
template <typename t_Domain,
          ...
          int t_PEZ = 1,
          int t_PEX = 1>
class RTMDomain : public Stencil3D<t_DataType, t_Order, t_MaxDimZ, t_MaxDimY, t_PEZ, t_PEX> {
   public:
    ...
    RTMDomain(int p_NZB = t_MaxB, int p_NYB = t_MaxB, int p_NXB = t_MaxB) {
#pragma HLS INLINE
#ifndef __SYNTHESIS__
        assert(t_MaxB >= p_NXB);
        assert(t_MaxB >= p_NYB);
