...
#include "rtmforward_domain_hbc.hpp"
...
#if RTM_numFSMs == 1
    s[0].forward(p_src[0], l_upb[0], l_vt_in, l_vt_out, l_pp_in, l_p[0], l_pp_out, l_p[1]);
#else
...
#pragma HLS RESOURCE variable = l_pp core = fifo_uram
    ...
    s[0].forward(p_src[0], l_upb[0], l_vt_in, l_vt[0], l_pp_in, l_p[0], l_pp[0], l_p[1]);
    for (int i = 1; i < RTM_numFSMs - 1; i++) {
#pragma HLS UNROLL
        s[i].forward(p_src[i], l_upb[i], l_vt[i - 1], l_vt[i], l_pp[i - 1], l_p[i], l_pp[i], l_p[i + 1]);
    }
    s[RTM_numFSMs - 1].forward(p_src[RTM_numFSMs - 1], l_upb[RTM_numFSMs - 1], l_vt[RTM_numFSMs - 2], l_vt_out,
                               l_pp[RTM_numFSMs - 2], l_p[RTM_numFSMs - 1], l_pp_out, l_p[RTM_numFSMs]);
#endif
    dataConsumer(l_entries, l_vt_out);

    s[0].saveUpb<RTM_numFSMs>(p_t, l_upb, p_upb);
...
extern "C" void rtmforward(const unsigned int p_z,
    ...
    POINTER(p_taperx, gmemParam)
    POINTER(p_tapery, gmemParam)
    POINTER(p_taperz, gmemParam)
    POINTER(p_coefx, gmemParam)
    POINTER(p_coefy, gmemParam)
    POINTER(p_coefz, gmemParam)
    POINTER(p_src, gmemParam)
    POINTER(p_upb, gmemDDR)
