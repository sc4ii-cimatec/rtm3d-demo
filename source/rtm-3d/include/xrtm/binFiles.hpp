...
void readBin(const string filename, const streampos readSize, vector<T, A>& vec) {
    ifstream file(filename, ios::binary);
    file.unsetf(ios::skipws);
    streampos fileSize;
    file.seekg(0, ios::end);
...
template <typename T>
void readBin(const string filename, const streampos readSize, T* vec) {
    ifstream file(filename, ios::binary);
    file.unsetf(ios::skipws);
    streampos fileSize;
    file.seekg(0, ios::end);
...
template <typename T>
void writeBin(const string filename, const streampos writeSize, const T* vec) {
...
template <typename T, typename A>
void writeBin(const string filename, const streampos writeSize, vector<T, A>& vec) {
